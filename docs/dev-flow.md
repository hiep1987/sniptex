...
```bash
# Bump version trong 3 file (giữ đồng nhất). LƯU Ý: chỉ dùng SemVer
# numeric (X.Y.Z), KHÔNG kèm pre-release suffix vào version field —
# WiX MSI bundler chỉ cho numeric pre-release identifier ("0.0.2-1" OK,
# "0.0.2-dev" FAIL). Suffix dev/alpha/beta nằm Ở TAG NAME thôi.
#
...
# Tag suffix "-dev" / "-alpha" / "-beta" là CI routing label —
# không nằm trong version field 3 file trên.
git tag v0.0.2-dev
